...
def interpret_score(score):
    if score <= -9.0:
        return "Very strong binding - likely active site engagement with multiple key residues.", LIGHT
    elif score <= -7.0:
        return "Moderate-strong binding - promising candidate for further optimization.", SECONDARY
    elif score <= -5.0:
        return "Weak-moderate binding - limited interactions, poor shape complementarity.", SECONDARY
    else:
        return "Poor binding affinity - ligand unlikely to be a viable inhibitor.", DARK
...
class KI1199App(ttk.Window):
    ...
    def _configure_styles(self):
        ...
        s.configure("Title.TLabel",       font=(FONT_FAMILY, 22, "bold"), foreground=ACCENT)
        s.configure("SectionHead.TLabel", font=(FONT_FAMILY, 12, "bold"), foreground=ACCENT)
        s.configure("Mono.TLabel",        font=(FONT_FAMILY, 10))
        s.configure("Score.TLabel",       font=(FONT_FAMILY, 18, "bold"), foreground=ACCENT)
        s.configure("Accent.TButton",     font=(FONT_FAMILY, 10, "bold"))
        s.configure(".", background=BG_DARK, foreground=ACCENT)
        s.configure("TFrame", background=BG_DARK)
        s.configure("TLabel", background=BG_DARK, foreground=ACCENT)
        s.configure("TNotebook", background=BG_DARK, borderwidth=0)
        s.configure("TNotebook.Tab", background=BG_CARD, foreground=ACCENT)
        s.map("TNotebook.Tab", background=[("selected", ACCENT)], foreground=[("selected", BG_DARK)])
        s.configure("TButton", background=BG_CARD, foreground=ACCENT, bordercolor=ACCENT)
        s.map("TButton", background=[("active", CHART_DARK), ("pressed", CHART_MID)])
        s.configure("TEntry", fieldbackground=BG_CARD, foreground=ACCENT, insertcolor=ACCENT)
        s.configure("TCombobox", fieldbackground=BG_CARD, foreground=ACCENT)
        s.configure("Treeview", background=BG_CARD, fieldbackground=BG_CARD, foreground=ACCENT)
        s.configure("Treeview.Heading", background=CHART_DARK, foreground=ACCENT)
        s.configure("Horizontal.TProgressbar", troughcolor=CHART_DARK, background=ACCENT, bordercolor=ACCENT)
    ...
    def _build_header(self):
        ...
        ttk.Label(hdr,
                  ...
                  font=self._font(9), foreground=FG_SOFT).pack(side=LEFT, padx=20)

        self.clock_var = tk.StringVar()
        ttk.Label(hdr, textvariable=self.clock_var,
                  font=self._font(9), foreground=FG_SOFT).pack(side=RIGHT)
        self._tick_clock()
    ...
    def _make_docking_tab(self):
        ...
        ttk.Entry(lf, textvariable=self.pdb_path, state="readonly",
                  font=self._font(9)).pack(fill=X, pady=(0, 4))
        ...
        ttk.Label(self.preset_frame, text="Select ligand:", font=self._font(9)).pack(anchor=W)
        ...
        self.preset_cb  = ttk.Combobox(self.preset_frame, textvariable=self.preset_var,
                                        ...
                                        state="readonly", font=self._font(9))
        ...
        ttk.Entry(self.upload_frame, textvariable=self.lig_path,
                  state="readonly", font=self._font(9)).pack(fill=X, pady=(0, 4))
        ...
        ttk.Label(self.smiles_frame, text="Enter SMILES:", font=self._font(9)).pack(anchor=W)
        self.smiles_var = tk.StringVar()
        ttk.Entry(self.smiles_frame, textvariable=self.smiles_var,
                  font=self._font(9)).pack(fill=X)
        ...
        for row_i, (label, key, default, tip) in enumerate(param_defs):
            ttk.Label(params_grid, text=label,
                      font=self._font(9)).grid(row=row_i, column=0, sticky=W, pady=2)
            ...
            ttk.Label(params_grid, text=tip,
                      font=self._font(8), foreground=FG_SOFT).grid(
                          row=row_i, column=2, sticky=W, padx=(6, 0))

        ttk.Separator(lf).pack(fill=X, pady=10)

        self.dock_btn = ttk.Button(lf, text="▶  RUN DOCKING",
                                   bootstyle=LIGHT, command=self._run_docking)
        self.dock_btn.pack(fill=X, ipady=8)

        self.dock_progress = ttk.Progressbar(lf, bootstyle=LIGHT,
        ...
        ttk.Label(lf, textvariable=self.dock_status,
                  font=self._font(9), foreground=FG_SOFT).pack(anchor=W)
        ...
        ttk.Label(score_card, text="DOCKING SCORE ",
                  font=self._font(9, "bold"), foreground=ACCENT).pack(anchor=W)
        ...
        ttk.Label(_sc_inner, textvariable=self.interp_var,
                  font=self._font(10), wraplength=500,
    ...
    def _make_poses_panel(self, parent):
        ...
        self.poses_tree = ttk.Treeview(
            ...
            show="headings", bootstyle=SECONDARY, height=10
        )
        for col, hdr, w in [
            ("rank",    "Rank",       60),
        ...
        ttk.Label(_det_inner, textvariable=self.pose_detail_var,
                  font=self._font(9), wraplength=620, justify=LEFT).pack(anchor=W)
    ...
    def _make_interact_panel(self, parent):
        ...
        for col, label, var, style in [
            (0, "H-Bonds",       self.hbond_var, SECONDARY),
            (1, "Van der Waals", self.vdw_var,   SECONDARY),
            (2, "Hydrophobic",   self.hphob_var, SECONDARY),
        ]:
            c = ttk.Frame(cards)
    ...
    def _make_chart_panel(self, parent):
        ...
        if HAS_MPL:
            ...
            ttk.Label(frame,
                      ...
                      font=self._font(11)).pack(expand=True)
        return frame
    ...
    def _make_screening_tab(self):
        ...
        ttk.Label(lf, text="Choose one or more ligands from the preset library:",
                  font=self._font(9), wraplength=280).pack(anchor=W, pady=(0, 6))
        ...
        ttk.Label(lf, text="Exhaustiveness:", font=self._font(9)).pack(anchor=W)
        self.screen_exhaust = tk.StringVar(value="8")
        ttk.Entry(lf, textvariable=self.screen_exhaust,
                  font=self._font(9)).pack(fill=X, pady=(0, 6))

        ttk.Separator(lf).pack(fill=X, pady=10)
        self.screen_btn = ttk.Button(lf, text="▶  RUN SCREEN",
                                     bootstyle=LIGHT, command=self._run_screening)
        self.screen_btn.pack(fill=X, ipady=8)
        self.screen_progress = ttk.Progressbar(lf, bootstyle=LIGHT,
                                               mode="determinate", maximum=100)
        ...
        self.screen_tree = ttk.Treeview(
            ...
            show="headings", bootstyle=SECONDARY, height=15
        )
        for col, hdr, w in [
            ("rank",    "#",               50),
            ("ligand",  "Ligand",         250),
        ...
        ttk.Label(_bc_inner, textvariable=self.best_var,
                  font=self._font(11, "bold"), foreground=ACCENT,
    ...
    def _make_mutation_tab(self):
        ...
        ttk.Label(lf, text="Compare wild-type vs mutant KI1199 docking scores.",
                  font=self._font(9), wraplength=280).pack(anchor=W, pady=(0, 8))

        ttk.Label(lf, text="Select Mutation:", font=self._font(9)).pack(anchor=W)
        self.mut_var = tk.StringVar(value=MUTATIONS[0])
        ttk.Combobox(lf, textvariable=self.mut_var, values=MUTATIONS,
                     state="readonly", font=self._font(9)).pack(fill=X, pady=(0, 8))

        ttk.Label(lf, text="Select Ligand:", font=self._font(9)).pack(anchor=W)
        ...
        self.mut_btn = ttk.Button(lf, text="▶  COMPARE WT vs MUTANT",
                                  bootstyle=LIGHT, command=self._run_mutation)
        self.mut_btn.pack(fill=X, ipady=8)
        self.mut_progress = ttk.Progressbar(lf, bootstyle=LIGHT,
                                            mode="determinate", maximum=100)
        ...
        ttk.Label(_mt_inner, textvariable=self.mt_score_var,
                  font=self._font(20, "bold"), foreground=ACCENT2).pack()
        ...
        ttk.Label(delta_card, text=" ΔΔBINDING ENERGY ",
                  font=self._font(9, "bold"), foreground=FG_SOFT).pack(anchor=W)
        ...
        ttk.Label(_delta_inner, textvariable=self.delta_var,
                  font=self._font(16, "bold")).pack(pady=(4, 0))
        ttk.Label(_delta_inner, textvariable=self.delta_interp,
                  font=self._font(10), wraplength=500).pack(pady=(8, 0))
    ...
    def _make_heatmap_tab(self):
        ...
        if HAS_MPL:
            ...
            ttk.Label(root,
                      text=" matplotlib not installed.\nRun: pip install matplotlib numpy",
                      font=self._font(12)).grid(row=1, column=0)
    ...
    def _make_history_tab(self):
        ...
        ttk.Button(top, text="  Clear", bootstyle=LIGHT,
    ...
    def _build_statusbar(self):
        ...
        ttk.Label(bar, textvariable=self.status_var,
                  font=self._font(9), foreground=FG_SOFT).pack(side=LEFT)
    ...
    def _plot_poses(self, poses):
        ...
        colors = [CHART_LIGHT if s <= -7 else CHART_MID if s <= -5 else CHART_DARK for s in scores]
        self.ax.barh(ranks, scores, color=colors, edgecolor="#ffffff22")
        self.ax.set_xlabel("Binding Affinity (kcal/mol)", color=FG_MUTED,
                           fontfamily=MPL_FONT_FAMILY)
        self.ax.set_ylabel("Pose Rank", color=FG_MUTED, fontfamily=MPL_FONT_FAMILY)
        self.ax.set_title("Docking Pose Scores", color=ACCENT,
                          fontfamily=MPL_FONT_FAMILY, fontsize=11)
        self.ax.tick_params(colors=FG_MUTED)
        self.ax.invert_yaxis()
        self.ax.axvline(-7, color=CHART_MID, linestyle="--", linewidth=0.8, alpha=0.8)
        self.ax.axvline(-9, color=ACCENT, linestyle="--", linewidth=0.8, alpha=0.8)
    ...
    def _display_mutation(self, lig, mut, wt, mt):
        ...
        if HAS_MPL:
            ...
            self.mut_ax.bar(["Wild-Type", f"Mutant ({mut})"], [ws, ms],
                            color=[CHART_LIGHT, CHART_MID], edgecolor="#ffffff22", width=0.4)
            self.mut_ax.set_ylabel("Binding Affinity (kcal/mol)", color=FG_MUTED,
                                   fontfamily=MPL_FONT_FAMILY)
            self.mut_ax.set_title(f"{lig}  WT vs {mut}", color=ACCENT,
                                  fontfamily=MPL_FONT_FAMILY, fontsize=9)
            self.mut_ax.tick_params(colors=FG_MUTED)
    ...
    def _generate_heatmap(self):
        ...
        im = self.heatmap_ax.imshow(scores, cmap="RdYlGn", aspect="auto",
                                    vmin=-11, vmax=-4)
        colorbar = self.heatmap_fig.colorbar(im, ax=self.heatmap_ax, label="kcal/mol",
                                   fraction=0.03)
        colorbar.ax.yaxis.set_tick_params(color=FG_MUTED)
        colorbar.outline.set_edgecolor(BORDER)
        plt.setp(colorbar.ax.get_yticklabels(), color=FG_MUTED, fontfamily=MPL_FONT_FAMILY)
        ...
        self.heatmap_ax.set_xticklabels(
            ...
            color=FG_MUTED, fontfamily=MPL_FONT_FAMILY, fontsize=8
        )
        self.heatmap_ax.set_yticks(range(len(mutations)))
        self.heatmap_ax.set_yticklabels(mutations, color=FG_MUTED,
                                         fontfamily=MPL_FONT_FAMILY, fontsize=9)
        self.heatmap_ax.set_title("Binding Affinity Heatmap — KI1199 Variants × Ligands",
                                   color=ACCENT, fontfamily=MPL_FONT_FAMILY, fontsize=11)

        for i in range(len(mutations)):
            for j in range(len(ligands)):
                text_color = "white" if scores[i, j] <= -7.5 else "black"
